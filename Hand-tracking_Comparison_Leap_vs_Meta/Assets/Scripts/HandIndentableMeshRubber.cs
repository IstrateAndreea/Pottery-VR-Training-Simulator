...
public class HandIndentableMeshRubber : MonoBehaviour
{
    ...
    public float retractSpeed = 4.0f;
    ...
    // Helper to track which verts were touched this frame
    private bool[] vertexTouchedThisFrame;
    ...
    void Start()
    {
        ...
        vertexTouchedThisFrame = new bool[deformedVertices.Length];
    ...
    void Update()
    {
        ...
        // Reset "touched" status for this frame
        for (int v = 0; v < vertexTouchedThisFrame.Length; ++v)
            vertexTouchedThisFrame[v] = false;
            ...
            for (int i = 0; i < deformedVertices.Length; ++i)
            {
                ...
                if (dist < indentRadius)
                {
                    ...
                    if (newDepth > originalDepth)
                    {
                        ...
                        vertexTouchedThisFrame[i] = true; // mark vertex as touched
                    }
                }
            }
        ...
        // Retract untouched vertices toward original position
        for (int i = 0; i < deformedVertices.Length; ++i)
        {
            if (!vertexTouchedThisFrame[i])
            {
                deformedVertices[i] = Vector3.Lerp(deformedVertices[i], originalVertices[i], Time.deltaTime * retractSpeed);
            }
        }
